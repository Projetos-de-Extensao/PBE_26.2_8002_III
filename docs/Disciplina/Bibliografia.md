---
id: bibliografia
title: Bibliografia
---

# Bibliografia e Referências

## 1. Referências Metodológicas e Técnicas

* **Processo Unificado (RUP / UP)**:
  * LARMAN, Craig. *Utilizando UML e Padrões: Uma introdução à análise e ao projeto orientados a objetos e ao desenvolvimento iterativo*. 3ª ed. Porto Alegre: Bookman, 2007.
  * KRUCHTEN, Philippe. *The Rational Unified Process: An Introduction*. 3rd ed. Addison-Wesley, 2003.

* **Engenharia de Software e Modelagem**:
  * SOMMERVILLE, Ian. *Software Engineering*. 10th ed. Pearson, 2015.
  * PRESSMAN, Roger S.; MAXIM, Bruce R. *Engenharia de Software: Uma Abordagem Profissional*. 8ª ed. McGraw-Hill, 2016.
  * FOWLER, Martin. *UML Distilled: A Brief Guide to the Standard Object Modeling Language*. 3rd ed. Addison-Wesley, 2003.

* **Frameworks e Tecnologias**:
  * DJANGO SOFTWARE FOUNDATION. *Django Documentation (v5.x)*. Disponível em: <https://docs.djangoproject.com/>.
    * PLANTUML. *PlantUML Language Reference Guide*. Disponível em: <https://plantuml.com/>.

---

## 2. Referências de Negócio e Domínio

* CARVALHO, Jonh. *Cenário de Aula: Backend GAAP — Gestão de Atletas de Alta Performance*. Ibmec, 2026.2.
* AGILE BUSINESS CONSORTIUM. *MoSCoW Prioritisation Guide*. DSDM Framework.
